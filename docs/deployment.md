...
3. Create schema / table using Session 3 scripts (run once against the remote URL):

   ```bash
   cd ai-session-3
   # DATABASE_URL=postgresql://...neon.tech/... npm / node step1-setup.js
   # DATABASE_URL=... node step2-ingest.js
   ```
...
**Embeddings:** Session ingest uses `@xenova/transformers` locally when you run `step2-ingest.js`. Run ingest from a machine with enough RAM (laptop or a one-off job), not necessarily from the slim API container on first boot.
